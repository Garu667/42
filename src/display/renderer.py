...
class MlxRenderer:
    ...
    def _draw_cell(
        ...
        if cell_val & SOUTH:
            self._fill_rect(px, py + self._cell_size - WALL_SIZE,
                            self._cell_size, WALL_SIZE, wall)
        if cell_val & EAST:
            self._fill_rect(px + self._cell_size - WALL_SIZE, py,
                            WALL_SIZE, self._cell_size, wall)
